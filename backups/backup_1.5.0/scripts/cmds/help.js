...
const doNotDelete = "【 𝗧𝗘𝗧𝗢 𝗖𝗛𝗔𝗧𝗕𝗢𝗧 0401 】";
/** 
* @author NTKhang
...
module.exports = {
	...
	langs: {
		...
		en: {
			help: "𝗖𝗼𝗺𝗺𝗮𝗻𝗱 𝗟𝗶𝘀𝘁\n────────\nHeres the Command List have fun using Teto(≧▽≦)\n────────\n%1\n𝗣𝗮𝗴𝗲 [ %2/%3 ]\n𝗧𝗼𝘁𝗮𝗹 𝗖𝗼𝗺𝗺𝗮𝗻𝗱𝘀: %4\n────────\n %6\n────────",
		...
		if (!command && !args[0] || !isNaN(args[0])) {
			...
			if (sortHelp == "name") {
				...
				for (const [name, value] of commands) {
					...
					if (shortDescription)
						describe += `\n-${cropContent(shortDescription.charAt(0).toLowerCase() + shortDescription.slice(1))}\n────────`;
					arrayInfo.push({
						data: describe,
